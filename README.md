...
A common data model (CDM) defines a set of standard locations in which information of specific types should be stored.  As such, it describes the end result of an extract, transform, and load (ETL) process for an arbitrary source (or raw) dataset.  CDM specifications can be created with a variety of goals for the resulting data.  The goals of this CDM are twofold:  

1. To minimize vocabulary mapping and restructuring in order to improve CDM adoption and separate the data model from the vocabulary model
2. To capture the provenance of the data in order to enhance the reproducibility of studies that use the CDM 

While we are inspired by CDMs such as OMOP, we find it requires a substantial mapping process (vocabularies and domains) that is intended to allow queries to operate on data from any source.  While this is a potentially powerful design goal, it results in a very complicated ETL process that varies across providers.  It creates a steep learning curve for understanding and using novel vocabularies required to navigate the data.  And it also ignores a substantial literature that expresses clinical research using validated algorithms based on the vocabularies in the source data.  This reduces transparency and reproducibility.  

One limitation to working exclusively in a common vocabulary is that there is limited information about the sensitivity and specificity of algorithms derived this way.  Further more, it is not important for us to encode, in the data, whether a particular code represents a condition, measurement, observation, or drug exposure.  Instead, we prefer algorithms expressed in their native vocabularies (e.g., ICD-9, HCPCS, CPT, etc.) so that we can focus on when, and in what context, the ideas represented by the codes were reported.   

The fundamental premise of our CDM is that storing the data in a CDM should be separate from mapping the data to clinical research variables via algorithms.  This is because vocabulary mappings are prone to revision and they can obscure the connection back to the source data ("provenance").  Furthermore, to the extent that there are potential efficiencies to expressing queries in a common vocabulary (e.g., SNOMED), this does not require that every code in the data be remapped during the ETL process.  It only requires a mapping from the common vocabulary (e.g., SNOMED) to the codes available in the dataset at hand, which can readily be done "on the fly."
